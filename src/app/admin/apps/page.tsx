...
import { getAppsAction } from '@/app/admin/actions/fetchers';
import { saveAppAction, deleteAppAction } from '@/app/admin/actions/apps';
import { reorderItemsAction } from '@/app/admin/actions/reorder';
import { SortableList } from '@/components/admin/sortable-list';
import { StatusBadge } from '@/components/admin/status-badge';
import { Button } from '@/components/ui/button';
import { Layers, Plus, Trash2, Edit3, X, Loader2 } from 'lucide-react';
...
export default function AppsEditorPage() {
    ...
    async function loadData() {
        const list = await getAppsAction();
        setApps(list || []);
        setLoading(false);
    }
    ...
    async function handleSave(e: React.FormEvent) {
        e.preventDefault();
        const res = await saveAppAction(form);
        if (res.success) {
            setModalOpen(false);
            loadData();
        } else {
            alert(res.message || 'Failed to save app');
        }
    }

    async function handleDelete(id: string) {
        if (!confirm('Are you sure you want to delete this tool badge?')) return;
    ...
    async function handleReorder(newItems: any[]) {
        ...
        await reorderItemsAction('apps', reorderPayload);
    }
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                        <Layers className="h-6 w-6 text-primary" /> Apps & Tools Showcase
                    </h1>
                    <p className="text-xs text-zinc-400 mt-1">Manage software and tool badges displayed in the tools marquee</p>
                </div>
                <Button variant="neon" size="sm" onClick={() => handleOpenModal()}>
                    <Plus className="h-4 w-4 mr-1" /> Add Tool Badge
                </Button>
            </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-6">
                        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                            <h2 className="text-lg font-bold text-white">
                                {editingItem ? 'Edit Tool Badge' : 'Add Tool Badge'}
                            </h2>
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div className="space-y-1">
                                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">App / Tool Name</label>
                                <input
                                    type="text"
                                    required
                                    value={form.name}
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button type="submit" variant="neon" size="sm">Save Badge</Button>
                                </div>
                            </div>
                        </form>
                    </div>
